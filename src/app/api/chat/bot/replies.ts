...
function fuzzyMatchesInput(input: string, trigger: string): boolean {
  ...
  const triggerWords = normalizedTrigger.split(/\s+/).filter((word) => word.length >= 3);
  if (!triggerWords.length) return false;
  const wordMatches = (target: string) =>
    input.includes(target) ||
    inputWords.some((word) => levenshtein(word, target) <= maxEditDistance(target));
  if (triggerWords.length === 1) return wordMatches(triggerWords[0]);
...
function shouldPreferProjectReply(input: string, topic: TrainingTopicId | null): boolean {
  if (!topic || topic === "projects") return true;
  const project = findProjectMatch(input);
  if (!project) return false;
  const normalized = normalizeInput(input);
  const titleWords = normalizeInput(project.title).split(/\s+/).filter((word) => word.length > 3);
