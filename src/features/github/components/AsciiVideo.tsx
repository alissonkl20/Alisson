...
import {
  buildTextLayout,
  lineBudgetCost,
  terminalHeightForLayout,
  type TextLayout,
  typewriterBudget,
} from "./ascii/layout";
import { buildTerminalShape } from "./ascii/terminal-shape";
...
export function AsciiVideo({ src = DEFAULT_VIDEO_SRC, className = "" }: AsciiVideoProps) {
  ...
  useEffect(() => {
    ...
    const drawTypedText = () => {
      ...
      let budget = typewriterBudget(modeTime);
      ...
      for (let li = 0; li < wrapped.length; li++) {
        ...
        budget -= lineBudgetCost(line.text.length);
      }
